...
from .base import Flatten3D, View, VAEModule
...
class Encoder(BaseEncDec):
    def __init__(
        ...
        features.append(nn.Linear(output_channel*output_img_size*output_img_size, num_latents))
...
class GaussianEncoder(Encoder):
    ...
    def __init__(
        ...
        super().__init__(num_latents * 2, num_channels, image_size, width_mult, inverted_residual_setting, block, norm_layer)
...
class Decoder(BaseEncDec):
    def __init__(
        ...
        def conv_layer(*args, **kwargs):
            stride = (kwargs.get('stride') or args[3])
            if stride > 1:
                kwargs['output_padding'] = stride - 1
                return nn.ConvTranspose2d(*args, **kwargs)
            else:
                return nn.Conv2d(*args, **kwargs)

        start_img_size = image_size >> 3
        ...
        features: List[nn.Module] = [
            nn.Linear(num_latents, input_channel*start_img_size*start_img_size),
            View((-1, input_channel, start_img_size, start_img_size))
        ]
        ...
        features.append(conv_layer(input_channel, num_channels, 1, 1))
...
class InvertedResidualVAEModel(VAEModule):
    def __init__(self, num_latents: int, num_channels: int = 3, image_size: int = 64):
        super().__init__(GaussianEncoder(num_latents, num_channels, image_size), Decoder(num_latents, num_channels, image_size))

        self.use_model_regulariser = True
        self.all_bn_layers = []
